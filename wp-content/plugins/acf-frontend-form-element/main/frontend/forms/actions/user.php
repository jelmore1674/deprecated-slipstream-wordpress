...
class ActionUser extends ActionBase {
	...
	public function get_default_fields( $form ){
		$default_fields = array(
			'username', 'user_email', 'user_password', 'first_name', 'last_name', 'role',
		);
			...
			case 'delete_user':
				...
				if( $form['user_to_edit'] == 'url_query' ){
					if( isset( $_GET[ $form['url_query_user'] ] ) ){
						...
						$form['url_query'][$form['url_query_user']] = $form['user_id'];
					}
				}
				...
				if( empty( $form['user_id'] ) || ! acf_frontend_user_exists( $form['user_id'] ) ) $form['user_id'] = 'none';
				break;
		}
		return $form;
	}
