...
class Frontend_Forms_UI
{
    ...
    function display_shortcode()
    {
        ...
        if ( 'admin_form' == $post->post_type ) {
            ...
            foreach ( array( 'form' ) as $content ) {
                echo  '<div class="shortcode-copy">' ;
                // Show shortcode
                $form_shortcode = "[" . FEA_PREFIX . " " . $content . "=\"" . $post->ID . "\"]";
                ...
                echo  '<button type="button" data-prefix="' . FEA_PREFIX . ' ' . $content . '" data-form="' . $post->ID . '" class="copy-shortcode"> ', $icon_path, ' ' . __( 'Copy Code', FEA_NS ) . '</button>' ;
                /* echo '<a target="_blank" href="' . site_url( "?page=".FEA_PRE."-forms&id={$post->ID}&action=preview" ) . '" class="button-primary">'. __( 'Preview', FEA_NS ) . '</a>'; */
                echo  '</div>' ;
            }
            echo  '</div>' ;
        }
    ...
    function columns_content( $column, $post_id )
    {
        ...
        if ( 'shortcode' == $column ) {
            // Show shortcode
            echo  "<code>[" . FEA_PREFIX . " form=\"" . $post_id . "\"]</code>" ;
            //Save icon location
            $icon_path = '<span class="dashicons dashicons-admin-page"></span>';
            // Add the button to the page.
            // The form code is saved into the button html for the JS above
            echo  '<button type="button" class="copy-shortcode" data-prefix="' . FEA_PREFIX . ' form" data-form="' . $post_id . '">', $icon_path, ' ' . __( 'Copy Code', FEA_NS ) . '</button>' ;
        }
    ...
    function admin_form_display( $content )
    {
        ...
        $form_preview = true;
        if ( $post->post_type == 'admin_form' ) {
            $content = '[' . FEA_PREFIX . ' form="' . $post->ID . '"]';
        }
        return $content;
    }
